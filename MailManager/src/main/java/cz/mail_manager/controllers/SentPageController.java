...
public class SentPageController {
	...
	public String submit(Model model) {
		...
		List<Email> emails = receivedEmails.getEmailsHeader("sent", 0);
	...
	public @ResponseBody Email getEmailContent (Model model, int detailIndex) {
		...
		Email email = receivedEmails.getEmailDetail("sent", detailIndex);
	...
	public @ResponseBody void moveEmail(int detailIndex, String folder) {
		...
		receivedEmails.move("sent", detailIndex, folder);
	}
	...
	public @ResponseBody void deleteEmail(int detailIndex) {
		...
		receivedEmails.delete("sent", detailIndex);
	}
	...
	public @ResponseBody List<Email> getMoreEmails(int lastIndex) {
		...
		List<Email> emails = receivedEmails.getEmailsHeader("sent", lastIndex);
	...
	public @ResponseBody List<Email> refresh(String sentDate) {
		...
		List<Email> emails = receivedEmails.refreshEmailsHeader("sent", sentDate);
