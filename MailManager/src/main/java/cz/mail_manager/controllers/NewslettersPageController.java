...
public class NewslettersPageController {
	 ...
	 */
	@RequestMapping(value = "/hromadne", method = RequestMethod.GET)
	public String submit(@ModelAttribute("user") User user, Model model) {
		
		ReceivedEmails serverConnection = new ReceivedEmails(user);
		
		List<Email> emails = serverConnection.getEmailsHeader("newsletters", 0);
	 ...
	 */
	@RequestMapping(value = "/hromadne/detail", method = RequestMethod.POST)
	public @ResponseBody Email getEmailContent (@ModelAttribute("user") User user, Model model, int detailIndex) {
		
		Email email = new ReceivedEmails(user).getEmailDetail("newsletters", detailIndex);
	 ...
	 * 	@param response
	 */
	@RequestMapping(value = "/hromadne/priloha${index}/*", method = RequestMethod.GET)
	 ...
	 */
	@RequestMapping(value = "/hromadne/presun", method = RequestMethod.POST)
	public @ResponseBody void moveEmail(@ModelAttribute("user") User user, int detailIndex, String folder) {
		
		new ReceivedEmails(user).move("newsletters", detailIndex, folder);
	}
	 ...
	 * 	@param detailIndex - index vybraného emailu
	 */
	@RequestMapping(value = "/hromadne/smazat", method = RequestMethod.POST)
	public @ResponseBody void deleteEmail(@ModelAttribute("user") User user, int detailIndex) {
		
		new ReceivedEmails(user).delete("newsletters", detailIndex);
	}
	 ...
	 */
	@RequestMapping(value = "/hromadne/nacteni", method = RequestMethod.POST)
	public @ResponseBody List<Email> getMoreEmails(@ModelAttribute("user") User user, int lastIndex) {
		
		List<Email> emails = new ReceivedEmails(user).getEmailsHeader("newsletters", lastIndex);
	 ...
	 */
	@RequestMapping(value = "/hromadne/obnoveni", method = RequestMethod.POST)
	public @ResponseBody List<Email> refresh(@ModelAttribute("user") User user, String sentDate) {
		
		List<Email> emails = new ReceivedEmails(user).refreshEmailsHeader("newsletters", sentDate);
