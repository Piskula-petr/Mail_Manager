...
public class DraftsPageController {
	 ...
	 */
	@RequestMapping(value = "/rozepsane/detail", method = RequestMethod.POST)
	public @ResponseBody Email getEmailContent (@ModelAttribute("user") User user, Model model, int detailIndex) {
		
		Email email = new ReceivedEmails(user).getEmailDetail("drafts", detailIndex);
	 ...
	 * 	@param response
	 */
	@RequestMapping(value = "/rozepsane/priloha${index}/*", method = RequestMethod.GET)
	 ...
	 */
	@RequestMapping(value = "/rozepsane/presun", method = RequestMethod.POST)
	public @ResponseBody void moveEmail(@ModelAttribute("user") User user, int detailIndex, String folder) {
		
		new ReceivedEmails(user).move("drafts", detailIndex, folder);
	}
	 ...
	 * 	@param detailIndex - index vybraného emailu
	 */
	@RequestMapping(value = "/rozepsane/smazat", method = RequestMethod.POST)
	public @ResponseBody void deleteEmail(@ModelAttribute("user") User user, int detailIndex) {
		
		new ReceivedEmails(user).delete("drafts", detailIndex);
	}
	 ...
	 */
	@RequestMapping(value = "/rozepsane/nacteni", method = RequestMethod.POST)
	public @ResponseBody List<Email> getMoreEmails(@ModelAttribute("user") User user, int lastIndex) {
		
		List<Email> emails = new ReceivedEmails(user).getEmailsHeader("drafts", lastIndex);
	 ...
	 */
	@RequestMapping(value = "/rozepsane/obnoveni", method = RequestMethod.POST)
	public @ResponseBody List<Email> refresh(@ModelAttribute("user") User user, String sentDate) {
		
		List<Email> emails = new ReceivedEmails(user).refreshEmailsHeader("drafts", sentDate);
